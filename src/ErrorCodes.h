...
#define ERR_INVALID_NUMBER                "Error 006: Invalid number '%s'.\n"
...
#define BAIL_IF_DATA_STACK_LESS_THAN(n)                                        \
  if (CellStackSize(&state->dataStack) < n) {                                  \
    fprintf(state->errorStream, ERR_MISSING_STACK_VALUES, n,                   \
            CellStackSize(&state->dataStack));                                 \
    return;                                                                    \
  }
